...
from modules.ui_prompt_styles import materialize_styles
        ...
        with gr.Row(elem_id=f"{id_part}_toprow", variant="compact"):
            ...
            with gr.Column(scale=1, elem_id=f"{id_part}_actions_column"):
                with gr.Row(elem_id=f"{id_part}_generate_box", elem_classes="generate-box"):
                    ...
                    self.hires_one = gr.Button("hires one", elem_id="txt2img_hires")
                ...
                with gr.Row(elem_id=f"{id_part}_tools"):
                    ...
                    self.materialize = ToolButton(value=apply_style_symbol, elem_id=f"{id_part}_style_apply", tooltip="Apply all selected styles from the style selction dropdown in main UI to the prompt.")
                ...
                self.materialize.click(
                    ...
                    inputs=[self.prompt, self.negative_prompt, self.ui_styles.dropdown],
                    outputs=[self.prompt, self.negative_prompt, self.ui_styles.dropdown],
                    show_progress=False,
                ).then(fn=None, _js="function(){update_"+id_part+"_tokens(); closePopup();}", show_progress=False)
...
def create_ui():
    ...
    with gr.Blocks(analytics_enabled=False) as txt2img_interface:
        ...
        with gr.Tab("Generation", id="txt2img_generation") as txt2img_generation_tab, ResizeHandleRow(equal_height=False):
            ...
            toprow.hires_one.click(
                on_hires,
                _js="(x, y, z, a, b) => [x, y, z, a, selected_gallery_index()]",
                inputs=[enable_hr, batch_count, scripts.scripts_txt2img.script('Seed').seed, generation_info, dummy_component],
                outputs=[enable_hr, batch_count, scripts.scripts_txt2img.script('Seed').seed, dummy_component],
                show_progress=False
            )
